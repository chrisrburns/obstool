...
if __name__ == '__main__':
    ...
    hover = HoverTool(tooltips=[("(R,T)", "(@r,@angles)")], names=['stars'])
    p = PolarPlot(plot_width=500, plot_height=500, min_border=0, clockwise=True,
          theta0=np.pi/2, rmax=2, tools=[hover])
    p.grid()
    p.taxis_label()
